...
public class DroolsUtils {
     ...
     * 默认规则文件所在路径
     */
    private static final String RULES_PATH = "alarm";
    ...
    private static List<File> getRuleFiles() throws IOException {
        ...
        String filePath = Thread.currentThread().getContextClassLoader().getResource("/com/xy/dev").getPath();
    ...
    public static InternalKieModule createKieJar(KieServices ks, ReleaseId releaseId, List<ResourceWrapper> resourceWrapper) {
        KieFileSystem kfs = createKieFileSystemWithKProject(ks, true);
        kfs.writePomXML(getPom(releaseId));
        for(ResourceWrapper resource : resourceWrapper){
            kfs.write("src/main/resources/" + resource.getTargetResourceName(), resource.getResource());
        }
        KieBuilder kieBuilder = ks.newKieBuilder(kfs);
    ...
    public static KieFileSystem createKieFileSystemWithKProject(KieServices ks, boolean isdefault) {
        ...
        KieBaseModel kieBaseModel1 = kproj.newKieBaseModel("alarmKB").setDefault(isdefault)
